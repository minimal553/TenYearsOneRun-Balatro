...
if ($metadata.id -ne 'ten_years_nine_grid' -or $metadata.version -ne '0.6.0') { throw 'Unexpected package identity/version.' }
...
$backup = Join-Path $projectRoot ('backups\installed-before-0.6-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Write-Output "INSTALLED 0.6.0: $targetMod"
